...
function loadPage()
{
 ...
 if( TABLE_SEARCH == "" || TABLE_SEARCH == null )
 {
  if( lastPage != TABLE_PAGE
   ...
   updateListeIrcServers();
 }
 else
  searchIrcServer( TABLE_SEARCH, TABLE_PAGE );
}
function updateListeIrcServers()
{
 var data = {
  action_form: "list",
 ...
 $.post( URI + "/system/ajax/intranet/ircservers.php",
  data,
  function( data )
  {
   //            if( errorCode( data ) )
   //            {
   $("#listeIrcServers").fadeOut( "fast",
    function()
    {
     $("#listeIrcServers").empty();
     $("#listeIrcServers").html( data.html );
     initContextMenu();
     $("#listeIrcServers").fadeIn( "fast" );

     $("#listeIrcServers").parent().find("h2 span.nbTotal span").empty();
     $("#listeIrcServers").parent().find("h2 span.nbTotal span").html( data.nbTotal );

     $("#navPagesIrcServers").empty();
     $("#navPagesIrcServers").html( data.navPages );
    } );
  //            }
  }, "json");
}
...
function searchIrcServer( w, page )
{
 if( ( $.trim(w) ).length < 3 )
 {
 ...
 $.post( URI + "/system/ajax/intranet/ircservers.php",
  ...
  function( data )
  {
   if( data.status == true )
   {
    ...
    {
     case 'add' :
      alert( "Le serveur IRC a été créée avec succès.",
      {
       submit: function(v,m)
       {
        window.location = URI + '/ircservers/';
        return false;
       }
      });
      break;
     case 'edit' :
      alert( "Le serveur IRC a été modifiée avec succès.",
      {
       submit: function(v,m)
       {
        window.location = URI + '/ircservers/';
...
function effacerIrcServer_confirm( id )
{
 alert( "Êtes-vous sûr de vouloir supprimer ce serveur IRC ?",
 {
  buttons: {
   Oui: true,
   Non: false,
...
function effacerIrcServer( id )
{
 ...
 $.post( URI + "/system/ajax/intranet/ircservers.php",
  ...
  function( data )
  {
   if (data.status == true)
   {
    alert( "Le serveur IRC a été supprimée avec succès." );
    updateListeIrcServers();
   } else {
    alert( "Une erreur est survenue." );
   }
 ...
 $('.context_menu').contextMenu( 'menu_contextuel',
 {
  ...
  bindings:
  {
   'context_menu_add': function(t){
    window.location= URI + '/ircservers/add/';
   },
   'context_menu_edit': function(t)
   {
    var id_element = getIdOfElement_contextMenu( t.id );
    window.location= URI + '/ircservers/edit/' + id_element + '/';
   },
   'context_menu_del': function(t){
    effacerIrcServer_confirm( getIdOfElement_contextMenu( t.id ) );
   },
