...
function loadPage()
{
 ...
 if( TABLE_SEARCH == "" || TABLE_SEARCH == null )
 {
  ...
  searchTeam( TABLE_SEARCH, TABLE_PAGE );
}
function updateListeTeams()
{
 var data = {
  action_form: "list",
 ...
 $.post( URI + "/system/ajax/intranet/teams.php",
  data,
  function( data )
  {
   //            if( errorCode( data ) )
   //            {
   $("#listeTeams").fadeOut( "fast",
    function()
    {
     $("#listeTeams").empty();
     $("#listeTeams").html( data.html );
     initContextMenu();
     $("#listeTeams").fadeIn( "fast" );

     $("#listeTeams").parent().find("h2 span.nbTotal span").empty();
     $("#listeTeams").parent().find("h2 span.nbTotal span").html( data.nbTotal );

     $("#navPagesTeams").empty();
     $("#navPagesTeams").html( data.navPages );
    } );
  //            }
  }, "json");
}
 ...
 $.post( URI + "/system/ajax/intranet/teams.php",
  ...
  function( data )
  {
   if( data.status == true )
   {
    ...
    {
     case 'add' :
      alert( "La team a été créés avec succès.",
      {
       submit: function(v,m)
       {
      ...
      break;
     case 'edit' :
      alert( "La team a été modifiés avec succès.",
      {
       submit: function(v,m)
       {
        window.location = URI + '/teams/';
...
function effacerTeam_confirm( id )
{
...
function effacerTeam( id )
{
 ...
 $.post( URI + "/system/ajax/intranet/teams.php",
  ...
  function( data )
  {
   if (data.status == true)
   {
    alert( "La team a été supprimée avec succès." );
    updateListeTeams();
   } else {
    alert( "Une erreur est survenue." );
   }
 ...
 $('.context_menu').contextMenu( 'menu_contextuel',
 {
  ...
  bindings:
  {
   'context_menu_add': function(t){
    window.location= URI + '/teams/add/';
   },
   'context_menu_edit': function(t)
   {
    var id_element = getIdOfElement_contextMenu( t.id );
    window.location= URI + '/teams/edit/' + id_element + '/';
   },
   'context_menu_del': function(t){
    effacerTeam_confirm( getIdOfElement_contextMenu( t.id ) );
   },
