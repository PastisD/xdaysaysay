...
function loadPage()
{
 ...
 if( TABLE_SEARCH == "" || TABLE_SEARCH == null )
 {
  ...
  searchXdcc( TABLE_SEARCH, TABLE_PAGE );
}
function updateListeXdccs()
{
 var data = {
  action_form: "list",
 ...
 $.post( URI + "/system/ajax/intranet/xdccs.php",
  data,
  function( data )
  {
   $("#listeXdccs").fadeOut( "fast",
    function()
    {
     $("#listeXdccs").empty();
     $("#listeXdccs").html( data.html );
     initContextMenu();
     $("#listeXdccs").fadeIn( "fast" );

     $("#listeXdccs").parent().find("h2 span.nbTotal span").empty();
     $("#listeXdccs").parent().find("h2 span.nbTotal span").html( data.nbTotal );

     $("#navPagesXdccs").empty();
     $("#navPagesXdccs").html( data.navPages );
    } );
  }, "json");
}
 ...
 $.post( URI + "/system/ajax/intranet/xdccs.php",
  ...
  function( data )
  {
   if( data.status == true )
   {
    ...
    {
     case 'add' :
      alert( "Le xdcc a été créé avec succès.",
      {
       submit: function(v,m)
       {
      ...
      break;
     case 'edit' :
      alert( "Le xdcc a été modifié avec succès.",
      {
       submit: function(v,m)
       {
        window.location = URI + '/xdccs/';
...
function effacerXdcc_confirm( id )
{
 alert( "Êtes-vous sûr de vouloir supprimer ce xdcc ?",
 {
  buttons: {
   Oui: true,
   Non: false,
...
function effacerXdcc( id )
{
 ...
 $.post( URI + "/system/ajax/intranet/xdccs.php",
  ...
  function( data )
  {
   if (data.status == true)
   {
    alert( "Le xdcc a été supprimée avec succès." );
    updateListeXdccs();
   } else {
    alert( "Une erreur est survenue." );
   }
 ...
 $('.context_menu').contextMenu( 'menu_contextuel',
 {
  ...
  bindings:
  {
   'context_menu_add': function(t){
    window.location= URI + '/xdccs/add/';
   },
   'context_menu_edit': function(t)
   {
    var id_element = getIdOfElement_contextMenu( t.id );
    window.location= URI + '/xdccs/edit/' + id_element + '/';
   },
   'context_menu_del': function(t){
    effacerXdcc_confirm( getIdOfElement_contextMenu( t.id ) );
   },
